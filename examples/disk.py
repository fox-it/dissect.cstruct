...
class Partition:
    def __init__(
        self, disk: BinaryIO, offset: int, size: int, vtype: int, name: str | None = None, guid: str | None = None
    ):
        self.disk = disk
        self.offset = offset
        self.size = size
...
def partitions(fh: BinaryIO, mbr: c_disk.mbr, offset: int) -> Iterator[Partition]:
    for mbr_p in mbr.part:
        ...
        yield Partition(fh, part_offset, mbr_p.sector_size * SECTOR_SIZE, mbr_p.type)
